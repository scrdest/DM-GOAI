...
/sense/sim_commander_eyes/proc/UpdatePerceptions(var/datum/goai/sim_commander/owner)
...
/sense/sim_commander_eyes/proc/SpotWaypoint(var/datum/goai/sim_commander/owner)
	if(isnull(owner))
		return
...
/sense/sim_commander_obstruction_handler/proc/SpotObstacles(var/datum/goai/sim_commander/owner)
	if(!owner)
		// No mob - no point.
		return
	...
	if(obstruction)
		...
		handled = owner.HandleWaypointObstruction(
			...
			move_handler = /datum/goai/sim_commander/proc/HandleDirectionalCoverLeapfrog,
			unique = FALSE,
			allow_failed = TRUE
		)
...
/sense/sim_commander_panic_pathfinder/proc/SpotObstacles(var/datum/goai/sim_commander/owner)
	if(!(owner))
		// No mob - no point.
		return
...
/sense/sim_commander_safespace_finder/proc/UpdateSafespace(var/datum/goai/sim_commander/owner)
	if(!owner)
		// useless in a vacuum
		return
...
/datum/goai/sim_commander/InitSenses()
	/* Parent stuff */
	. = ..()
