...
*/
/datum/goai/sim_commander/proc/ChoosePanicRunLandmark(var/atom/primary_threat = null, var/list/threats = null, min_safe_dist = null)
	if(!(src.pawn))
	...
	if(best_local_pos)
		...
		if(obstruction)
			...
			handled = HandleWaypointObstruction(
				...
				move_handler = /datum/goai/sim_commander/proc/HandlePanickedRun,
				unique = FALSE,
				allow_failed = TRUE
			)
...
*/
/datum/goai/sim_commander/proc/pureHandleChoosePanicRunLandmark(var/datum/goai/sim_commander/owner = null, var/atom/bestpos = null, var/min_safe_dist = 2, var/list/cached_threats = null)
	// Abstracted ownership, but defaults to src for convenience.
	var/datum/goai/sim_commander/true_owner = (owner || src)
...
*/
/datum/goai/sim_commander/proc/HandleChoosePanicRunLandmark(var/datum/ActionTracker/tracker)
...
/datum/goai/sim_commander/proc/fPanicRunDistance(var/S, var/T)
	if(!S || !T)
		return
...
/datum/goai/sim_commander/proc/HandlePanickedRun(var/datum/ActionTracker/tracker)
	if(!(src.pawn))
		world.log << "[src] does not have an owned mob!"
		return
	...
	*/
	...
	if(best_local_pos && (!src.active_path || src.active_path.target != best_local_pos))
		...
		StartNavigateTo(best_local_pos, 0, primary_threat?.loc, 0, /datum/goai/sim_commander/proc/fPanicRunDistance)
