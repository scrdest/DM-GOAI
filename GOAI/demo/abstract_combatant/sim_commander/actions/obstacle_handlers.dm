/datum/goai/sim_commander/proc/HandleOpenDoor(var/datum/ActionTracker/tracker)
	if(!(src.pawn))
...
/datum/goai/sim_commander/proc/HandleOpenAutodoor(var/datum/ActionTracker/tracker)
	if(!(src.pawn))
		world.log << "[src] does not have an owned mob!"
		return
