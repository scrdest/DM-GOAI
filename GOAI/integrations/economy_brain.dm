...
/datum/brain/utility/GetCommodityDesirability(var/commodity, var/amount, var/cash_value = 0, var/list/curr_need_level_overrides = null)
	...
	var/list/need_weights = src.GetNeedWeights() // assoc[str, float]
	if(!need_weights)
...
/datum/brain/utility/GetMoneyDesirability(var/amount, var/curr_wealth = null)
	...
	if(amount < 0)
		var/raw_reserves_desired = src.personality?[PERSONALITY_KEY_MONEY_RESERVE_GOAL]
...
/datum/brain/utility/GetMoneyForNeedUtility(var/utility, var/curr_wealth = null)
	/* This is effectively an inverse of GetMoneyDesirability(Amt).
	//
...
/datum/brain/utility/GetCommodityAmountForNeedDelta(var/commodity, var/need_key, var/delta, var/need_per_unit_override = null)
	/* Pre-flight checks */
...
/datum/brain/utility/GetBestPurchaseCommodityForNeed(var/need_key)
	. = ..(need_key)
...
/datum/brain/utility/GetBestSaleCommodityForNeed(var/need_key)
	. = ..(need_key)
