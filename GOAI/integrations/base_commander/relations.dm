...
/datum/goai/proc/SetRelationshipTag(var/tag, var/value = null, var/weight = null) // -> datum/relationships
...
/datum/goai/proc/DropRelationshipTag(var/tag) // -> datum/relationships
	/* This is little more than a wrapper over the Relationships datum's Insert,
	// with lazy initialization if SOMEHOW it wasn't initialized properly.
	*/
	if(!tag)
...
/datum/goai/proc/GetRelationshipTagsFor(var/atom/trg) // -> list
...
/datum/goai/proc/HookGetRelationshipTagsFor(var/atom/trg) // -> list | null
	// Hook for subclasses to add extra tags in as needed.
	return


/datum/goai/proc/CheckRelationsTo(var/atom/trg, var/datum/relationships/relationships_override = null) // -> /dict
	if(isnull(trg))
		return
...
/datum/goai/proc/IsNeutral(var/atom/trg, var/datum/relationships/relationships_override = null) // -> bool
	if(isnull(trg))
		return FALSE
...
/datum/goai/proc/IsEnemy(var/atom/trg, var/datum/relationships/relationships_override = null) // -> bool
	if(isnull(trg))
		return FALSE
...
/datum/goai/proc/IsFriend(var/atom/trg, var/datum/relationships/relationships_override = null) // -> bool
	if(isnull(trg))
		return FALSE
