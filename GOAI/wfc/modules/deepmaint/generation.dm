...
/area/map_template/deepmaint_wfc/proc/generate_deepmaint(var/ruleset = "deepmaint.json", var/mapname = "genmap.json")
	...
	if(isnull(min_z))
		...
		to_world("Deepmaint - failed to locate a valid Z-level!")
		return FALSE

	var/success = deepmaint_rebuild_mapfile(ruleset) && src.deepmaint_from_mapfile(min_z, mapname, overwrite_all = FALSE)
