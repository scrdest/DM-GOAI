...
/proc/deregister_ai_brain(var/id)
	...
	if(!(GOAI_LIBBED_GLOB_PREFIX(?.)global_aibrain_registry))
		return

	if(!(IS_REGISTERED_AI(id)))
		return

	var/datum/brain/aibrain = GOAI_LIBBED_GLOB_PREFIX(?.)global_aibrain_registry[id]
	...
	*/
	...
	GOAI_LIBBED_GLOB_PREFIX(?.)global_aibrain_registry[id] = null
...
/datum/brain/proc/RegisterBrain()
	...
	if(!(GOAI_LIBBED_GLOB_PREFIX(?.)global_aibrain_registry))
		GOAI_LIBBED_GLOB_PREFIX(?.)global_aibrain_registry = list()

	if(src.registry_index)
		// already done, fix up the registry to be sure and return
		GOAI_LIBBED_GLOB_PREFIX(?.)global_aibrain_registry[src.registry_index] = src
		return GOAI_LIBBED_GLOB_PREFIX(?.)global_aibrain_registry

	GOAI_LIBBED_GLOB_PREFIX(?.)global_aibrain_registry += src
	src.registry_index = GOAI_LIBBED_GLOB_PREFIX(?.)global_aibrain_registry.len
