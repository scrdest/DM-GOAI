...
# define GOAI_HAS_UTILITY_ACTIONS_BOILERPLATE_PROXIMITY_CHEBYSHEV(OBJTYPE, MAXDIST) ##OBJTYPE/HasUtilityActions(var/requester, var/list/args = null) { var/atom/requester_atom = requester; if(isnull(requester_atom)) { return FALSE }; if(ChebyshevDistance(requester_atom, src) > MAXDIST) { return FALSE }; return TRUE }

// Available if nearby (by Manhattan dist; requires source to be an atom!):
# define GOAI_HAS_UTILITY_ACTIONS_BOILERPLATE_PROXIMITY_MANHATTAN(OBJTYPE, MAXDIST) ##OBJTYPE/HasUtilityActions(var/requester, var/list/args = null) { var/atom/requester_atom = requester; if(isnull(requester_atom)) { return FALSE }; if(ManhattanDistance(requester_atom, src) > MAXDIST) { return FALSE }; return TRUE }
